...
@if ($resting)
    <div {{ $attributes->class('flex flex-col items-center justify-center gap-1.5 rounded-xl border border-dashed border-zinc-300 bg-zinc-50/60 px-4 py-6 text-center dark:border-white/15 dark:bg-white/[0.03]') }}>
...
@else
    ...
    <{{ $tag }}
        ...
        {{ $attributes->class('hover-lift group block rounded-xl border border-zinc-200 bg-white p-4 dark:border-white/10 glass-panel' . ($href ? ' cursor-pointer' : '')) }}
    >
        <div class="flex items-center justify-between gap-3">
            ...
            <div class="flex shrink-0 items-center gap-2 rounded-lg bg-zinc-100 px-3 py-1.5 dark:bg-white/10">
                <span class="text-lg font-bold tabular-nums {{ $scoreClasses }}">{{ $match->home_score ?? '–' }}</span>
                <span class="text-zinc-400 dark:text-white/30">-</span>
                <span class="text-lg font-bold tabular-nums {{ $scoreClasses }}">{{ $match->away_score ?? '–' }}</span>
            </div>
