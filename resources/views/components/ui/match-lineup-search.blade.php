...
@elseif ($candidates->isNotEmpty())
    ...
    {{-- Collapsed by default: this is the club's WHOLE roster to pick from,
         not the match's final call-up list below it -- starting it open
         made the two easy to confuse at a glance. --}}
    <div class="overflow-hidden rounded-2xl border border-zinc-200 dark:border-white/10 glass-panel" x-data="{ open: false, query: '' }">
        <button type="button" @click="open = !open" class="flex w-full cursor-pointer items-center justify-between gap-3 px-4 py-2.5 text-left transition-colors hover:bg-zinc-50 dark:hover:bg-white/5">
        ...
        <div x-show="open" x-collapse.duration.200ms>
            <form method="POST" action="{{ route('matches.lineups.store', $match) }}" class="border-t border-zinc-200 dark:border-white/10">
                @csrf
